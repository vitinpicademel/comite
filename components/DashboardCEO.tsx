...
import { exportarRelatorioExcel } from '@/lib/exportRelatorio'
...
export default function DashboardCEO({ socket, onBack }: { socket: Socket | null; onBack: () => void }) {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('avaliacaoRemovida', (data: { corretor: string }) => {
        // Remover avaliação quando corretor desconecta
        setAvaliacoes(prev => prev.filter(av => av.corretor !== data.corretor))
      })
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <button
                ...
                className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-all duration-300 shadow-lg shadow-green-600/50 hover:shadow-green-600/70 hover:scale-105 flex items-center gap-2"
                title="Exportar relatório do dia em Excel"
              >
                <Download className="w-5 h-5" />
                <span className="hidden sm:inline">Exportar Relatório do Dia</span>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-brown-dark/50 border border-brown-dark rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-brown-light focus:border-transparent transition-all"
                  >
