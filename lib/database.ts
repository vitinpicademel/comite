...
// Remover avaliação de um corretor específico para um imóvel (usado na desconexão)
export async function removerAvaliacaoCorretor(imovelId: string, corretor: string) {
  const { error } = await supabase
    .from('avaliacoes')
    .delete()
    .eq('imovel_id', imovelId)
